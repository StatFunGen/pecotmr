...
.js_makeSe <- function(
    ...
    names(rng) <- traits
    expr <- matrix(
        rnorm(length(traits) * length(samples)),
        nrow = length(traits),
        ncol = length(samples),
        dimnames = list(traits, samples)
    )
    cd <- S4Vectors::DataFrame(
        sex = rep(c("M", "F"), length.out = length(samples)),
        row.names = samples
    )
...
test_that(".sp* helpers: MultiStudyQtlDataset combines individual + sumstats", {
    ...
    ss <- .js_makeQtlSumStats(
        studies = "ssC",
        contexts = "DLPFC",
        traits = "ENSG_A"
    )
    mt <- MultiStudyQtlDataset(
        qtlDatasets = list(indA = qd1, indB = qd2),
        sumStats = ss
    )
    expect_setequal(pecotmr:::.spListStudies(mt), c("indA", "indB", "ssC"))
    expect_setequal(pecotmr:::.spListContexts(mt), c("brain", "liver", "DLPFC"))
...
test_that("parseJointSpecification: rejects unknown scope keys", {
    ...
    expect_error(
        pecotmr:::parseJointSpecification(
            list(list(axes = "context", scope = list(bogus = "x"))),
            qd
        ),
        "unknown scope key"
    )
...
test_that("parseJointSpecification: rejects unknown spec elements", {
    ...
    expect_error(
        pecotmr:::parseJointSpecification(
            list(list(axes = "context", scope = NULL, bogus = "x")),
            qd
        ),
        "unknown element"
    )
...
test_that("parseTraitIds: doubly-nested study->context validates per context", {
    ...
    out <- pecotmr:::parseTraitIds(
        list(A = list(brain = "ENSG_A", liver = "ENSG_B")),
        qd
    )
...
test_that("parseMethods: rejects multi-axis methods at per-trait level", {
    ...
    expect_error(
        pecotmr:::parseMethods(
            methods = list(
                A = list(brain = list(ENSG_A = c("susie", "mvsusie")))
            ),
            data = qd,
            caps = pecotmr:::.fineMappingMethodCapabilities,
            multivariateMethods = c("mvsusie", "fsusie")
...
.jd_mockMrmashRssWeights <- function() {
    function(stat, LD, ...) {
        ...
        rownames(w) <- if (is.matrix(stat$z)) {
            rownames(stat$z)
        } else {
            stat$variantNames
        }
...
.jd_makeSe <- function(traits = c("t1", "t2"), n_samples = 30L, starts = NULL) {
    if (is.null(starts)) {
...
test_that(".sp* helpers: QtlSumStats study/context/trait listing + dataForm", {
    ...
    expect_error(
        pecotmr:::.spStudyDataForm(ss, "missing"),
        "not in QtlSumStats"
    )
})
...
test_that("parseJointSpecification: malformed inputs error", {
    ...
    expect_error(
        pecotmr:::parseJointSpecification(list(list(scope = NULL)), qd),
        "missing `axes`"
    )
    expect_error(
        pecotmr:::parseJointSpecification(list(42), qd),
        "character vector or a named list"
    )
    expect_error(
        pecotmr:::parseJointSpecification(list(list(axes = character(0))), qd),
        "non-empty character"
    )
    expect_error(
        pecotmr:::parseJointSpecification(
            list(list(axes = "context", scope = c("a", "b"))),
            qd
        ),
        "named list"
    )
    expect_error(
        pecotmr:::parseJointSpecification(
            list(list(axes = "context", scope = list(study = integer(0)))),
            qd
        ),
...
test_that("parseContexts: malformed inputs error", {
    ...
    expect_error(
        pecotmr:::parseContexts(list(S1 = character(0)), qd),
        "non-empty character vector"
    )
...
test_that("parseTraitIds: malformed inputs error", {
    ...
    expect_error(
        pecotmr:::parseTraitIds(list(nope = "ENSG1"), qd),
        "unknown studies"
    )
    expect_error(
        pecotmr:::parseTraitIds(list(S1 = character(0)), qd),
        "non-empty character vector"
    )
    expect_error(
        pecotmr:::parseTraitIds(list(S1 = list("brain")), qd),
        "named by context"
    )
    expect_error(
        pecotmr:::parseTraitIds(list(S1 = list(brain = character(0))), qd),
        "non-empty character vector"
    )
    expect_error(
        pecotmr:::parseTraitIds(list(S1 = list(brain = "nope")), qd),
        "unknown traits"
    )
    expect_error(
        pecotmr:::parseTraitIds(list(S1 = 42), qd),
        "character vector or a named list"
    )
})
...
test_that(".spWalkMethods: structural errors via parseMethods", {
    ...
    expect_error(
        walk(list(S1 = list(brain = list(ENSG1 = list(x = "lasso"))))),
    ...
    expect_error(
        pecotmr:::.spWalkMethods(setNames(list(), character(0))),
        "empty named list"
    )
})
...
test_that("parseMethods: split-form and leaf validation errors", {
    ...
    expect_error(
        pecotmr:::parseMethods(
            methods = NULL,
            sumStatsMethods = character(0),
            qtlDatasetMethods = "lasso",
            data = qd,
            caps = caps,
    ...
    expect_error(
        pecotmr:::parseMethods(
            methods = NULL,
            sumStatsMethods = "lasso",
            qtlDatasetMethods = character(0),
            data = qd,
            caps = caps,
            multivariateMethods = "mrmash"
        ),
...
test_that("validateMethodsVsJointSpec: empty spec is a no-op; per-trait nesting + trait axis errors", {
    expect_null(pecotmr:::validateMethodsVsJointSpec(
        list(shape = "primary", methods = "lasso"),
        list()
    ))
    ...
    expect_error(
        pecotmr:::validateMethodsVsJointSpec(methodsParsed, parsed),
        "nests per-trait"
    )
})
...
test_that(".buildJointSumstatZMatrix: a mismatched SNP order across entries errors", {
    ...
    expect_error(
        pecotmr:::.buildJointSumstatZMatrix(
            df,
            c(1L, 2L),
            c("c1", "c2"),
...
test_that(".buildIndividualCrossContextXy: region path + complete-case skip", {
    ...
    local_mocked_bindings(
        ...
        .fmResidPheno = function(x, contexts, traitId = NULL, ...) {
            ...
            list(
                c1 = ym(c(NA, NA, NA, NA, NA, 1)), # mostly NA -> < 2 complete
                c2 = ym(rnorm(6))
            )
        },
        .package = "pecotmr"
...
test_that(".buildIndividualCrossContextXy: too few shared samples skips", {
    ...
    local_mocked_bindings(
        ...
        .fmResidPheno = function(x, contexts, traitId = NULL, ...) {
            list(
                c1 = matrix(0, 6, 1, dimnames = list(paste0("s", 1:6), "g1")),
...
test_that(".fmTraitsInRegion: filters traits by phenotype overlap with the region", {
    ...
    expect_equal(
        pecotmr:::.fmTraitsInRegion(se, c("g1", "g2"), NULL),
        c("g1", "g2")
    ) # NULL region -> unchanged
})
...
test_that(".enumerateComposedSumstatGroups: empty scope -> NULL; no complement -> one block", {
    ...
    gi <- pecotmr:::.enumerateComposedSumstatGroups(
        list(axes = c("study", "context", "trait")),
        df,
        list(
            studies = "S",
...
test_that(".buildComposedIndividualXy: disjoint samples / missing trait col / NA rows skip", {
    ...
    local_mocked_bindings(
        ...
        .fmResidPheno = function(x, contexts, traitId = NULL, ...) {
            list(
                c1 = matrix(0, 6, 1, dimnames = list(samp, "g1")),
                c2 = matrix(0, 6, 1, dimnames = list(samp, "zzz"))
            )
        }, # no g1/g2
        .package = "pecotmr"
    )
    expect_null(suppressMessages(pecotmr:::.buildComposedIndividualXy(
        NULL,
        list(contexts = list(S = c("c1", "c2")), traits = list(S = "g1")),
        "S",
        cisWindow = 1000L,
        verbose = 1,
        label = "X"
