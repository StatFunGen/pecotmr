...
test_that("input is validated", {
  d <- mk_xy()
  expect_error(cv(d$X, d$Y, fold = 0, fitFold = mock_fit_fold), "positive integer")
  expect_error(cv(d$X, d$Y, fold = "a", fitFold = mock_fit_fold), "positive integer")
  expect_error(cv(as.data.frame(d$X), d$Y, fold = 2, fitFold = mock_fit_fold),
               "X must be a matrix")
  expect_error(cv(d$X, d$Y[1:5, , drop = FALSE], fold = 2, fitFold = mock_fit_fold),
               "same")
  expect_error(cv(d$X, d$Y, fitFold = mock_fit_fold),
...
test_that("Y as a vector is converted to a matrix with a message", {
  d <- mk_xy(k = 1)
  expect_message(
    cv(d$X, as.numeric(d$Y), fold = 2, fitFold = mock_fit_fold, verbose = 1),
...
test_that("a provided samplePartition is reused; a fold mismatch warns", {
  d <- mk_xy(n = 12)
  sp <- data.frame(Sample = rownames(d$X), Fold = rep(1:3, each = 4),
                   stringsAsFactors = FALSE)
  r <- suppressMessages(cv(d$X, d$Y, samplePartitions = sp, fitFold = mock_fit_fold))
  expect_equal(r$samplePartition, sp)
  expect_message(
    cv(d$X, d$Y, fold = 2, samplePartitions = sp, fitFold = mock_fit_fold,
       verbose = 1),
    "does not match")
})
...
test_that("a samplePartition with unknown samples errors", {
  d <- mk_xy()
  sp <- data.frame(Sample = paste0("zzz", 1:5), Fold = rep(1:2, length.out = 5))
  expect_error(cv(d$X, d$Y, samplePartitions = sp, fitFold = mock_fit_fold),
               "do not match")
})
...
test_that("maxNumVariants subsamples variants with a message", {
  d <- mk_xy(p = 20)
  expect_message(
    cv(d$X, d$Y, fold = 2, fitFold = mock_fit_fold, maxNumVariants = 8,
       verbose = 1),
...
test_that("zero-variance predictions yield NA metrics with a message", {
  ...
  expect_message(r <- cv(d$X, d$Y, fold = 3, fitFold = zero_fit, verbose = 1),
                 "zero variance")
...
test_that("the parallel fold path matches the serial one", {
  d <- mk_xy(n = 40, seed = 7)
  set.seed(1)
  r1 <- suppressMessages(cv(d$X, d$Y, fold = 4, fitFold = mock_fit_fold,
                            numThreads = 1))
  set.seed(1)
  r2 <- suppressMessages(cv(d$X, d$Y, fold = 4, fitFold = mock_fit_fold,
                            numThreads = 2))
...
test_that("retainFits collects per-fold fits only when requested", {
  ...
  r_off <- suppressMessages(cv(d$X, d$Y, fold = 3, fitFold = fit_with_model,
                               retainFits = FALSE))
  expect_true(all(vapply(r_off$foldFits, length, integer(1)) == 0L))
  r_on <- suppressMessages(cv(d$X, d$Y, fold = 3, fitFold = fit_with_model,
                              retainFits = TRUE))
