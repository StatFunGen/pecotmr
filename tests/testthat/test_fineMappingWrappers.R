...
test_that("postprocessFinemappingFits keeps all effects when V is NULL", {
    ...
    post <- postprocessFinemappingFits(
        fits = list(
            susieRss = pecotmr:::.setFinemappingFitClass(
                fake_output,
                "susieRss"
            )
        ),
...
test_that("postprocessFinemappingFits stores outcome_names, coef, and clfsr for mvsusie", {
    ...
    post <- postprocessFinemappingFits(
        fits = list(
            mvsusie = pecotmr:::.setFinemappingFitClass(fake_output, "mvsusie")
        ),
        dataX = X,
        dataY = NULL,
        xScalar = 1,
...
.make_univariate_data <- function(
    ...
    if (length(effect_idx) > 0) {
        if (is.null(effect_size)) {
            effect_size <- rep(1.5, length(effect_idx))
        }
...
.fake_fit_and_cs <- function(
    ...
    if (n_variants == length(variant_ids)) {
...
test_that("buildTopLoci stamps per-condition lfsr + conditional_effect for mvsusie (long + numeric)", {
    ...
    alpha[1, 1:2] <- c(0.6, 0.3)
    mu <- array(0.3, c(L, p, R))
    mu2 <- array(1.2, c(L, p, R))
    pip <- c(0.6, 0.4, 0.05, 0.04)
    coef <- matrix(
        c(0.5, 0.4, 0.1, 0.05, 0.2, 0.1, 0.02, 0.01),
        p,
        R,
...
test_that("getTopLoci surfaces mvsusie conditional_effect + lfsr through the projector", {
    tl <- data.frame(
        ...
        A2 = c("G", "T"),
        pip = c(0.6, 0.4),
        posterior_mean = c(0.1, 0.2),
        posterior_sd = c(0.05, 0.05),
        cs_95 = c("mvsusie_1", "mvsusie_1"),
        conditional_effect = c(0.3, -0.2),
        lfsr = c(0.001, 0.02),
        stringsAsFactors = FALSE
    )
    e <- FineMappingEntry(
...
test_that("buildTopLoci derives CS columns from the ACTUAL coverages (no hardcoded 0.95/0.70/0.50)", {
    ...
    fit <- list(
        alpha = matrix(c(0.7, 0.2, 0.1), 1, 3),
        mu = matrix(0.3, 1, 3),
        mu2 = matrix(1.2, 1, 3),
        pip = c(0.7, 0.2, 0.1)
    )
    class(fit) <- "susie"
    mkCt <- function(cs, pu) {
        list(sets = list(cs = cs, purity = data.frame(min.abs.corr = pu)))
    }
    ...
    expect_true(all(
        c("cs_90", "cs_80", "cs_90_purity", "cs_80_purity") %in% names(tl)
    ))
...
test_that("buildTopLoci stamps per-variant logBF (max single-effect lbf; replaces log10_base_factor)", {
    ...
    fit <- list(
        alpha = matrix(c(0.7, 0.2, 0.1, 0.3, 0.4, 0.3), 2, 3),
        mu = matrix(0.3, 2, 3),
        mu2 = matrix(1.2, 2, 3),
        pip = c(0.7, 0.5, 0.3),
        lbf_variable = lbf
...
test_that("buildTopLoci emits 22 columns in the fixed order on a non-empty fit", {
    ...
    out <- .runBuildTopLoci(
        inp,
        method = "susie",
        sumstats = list(betahat = c(0.2, -0.1), sebetahat = c(0.05, 0.04)),
        af = c(0.10, 0.25),
...
test_that("buildTopLoci fullFit: within_cs_pip default + wide per-CS matrices", {
    ...
    lbf <- matrix(
        c(3, 2.5, 0.1, 0.1, 0, 0.1, 0.1, 2, 1.8, 0),
        L,
        P,
        byrow = TRUE
    )
    ...
    expect_setequal(
        grep("^within_cs_pip_", names(tl2), value = TRUE),
        c("within_cs_pip_cs1", "within_cs_pip_cs2")
    )
})

test_that("buildTopLoci fullFit on the committed qtlFineMappingExample (real CS)", {
    ...
    expect_equal(
        tl$within_cs_pip[match(vn[member], tl$variant_id)],
...
test_that("postprocessFinemappingFits forwards medianAbsCorr to susie_get_cs (OR-logic admits >= sets)", {
    ...
    pStrict <- postprocessFinemappingFits(
        list(susie = fit),
        dataX = d$X,
        dataY = d$y,
        coverage = 0.95,
...
test_that("per-method CS indices are independent across susie and susieInf (postprocessFinemappingFits)", {
    ...
    for (col in c("cs_95", "cs_70", "cs_50")) {
        expect_true(
            all(grepl("^susie_\\d+$", susie_rows[[col]])),
            info = paste("susie rows have wrong prefix in", col)
        )
        expect_true(
            all(grepl("^susie_inf_\\d+$", susie_inf_rows[[col]])),
...
test_that("overlapping CS across methods produces one row per method", {
    ...
    if (nrow(tl) > 0L) {
        ...
        if (length(shared) > 0L) {
            ...
            succeed(
                "no shared variants in this fixture; cross-method uniqueness rule is structural"
            )
        }
    } else {
        succeed(
            "empty top_loci from this fixture; cross-method uniqueness rule is structural"
        )
    }
})
...
test_that("postprocessFinemappingFits does not return top_loci_long anywhere", {
    ...
    for (name in names(post$finemappingResults)) {
        expect_false(
            "top_loci_long" %in% names(post$finemappingResults[[name]]),
            info = name
        )
    }
})
...
test_that("posterior_mean equals colSums(alpha*mu); posterior_sd equals sqrt(pmax(colSums(alpha*mu2) - mean^2, 0))", {
    ...
    expected_se <- sqrt(pmax(
        colSums(inp$fit$alpha * inp$fit$mu2) - expected_mean^2,
        0
    ))
    ...
    for (i in seq_along(variant_ids)) {
        ...
        expect_equal(
            unique(row$posterior_mean),
            expected_mean[i],
            tolerance = 1e-10
        )
        expect_equal(
            unique(row$posterior_sd),
            expected_se[i],
            tolerance = 1e-10
        )
    }
...
test_that("fsusieWrapper errors when fsusieR is not installed", {
    skip_if(
        requireNamespace("fsusieR", quietly = TRUE),
...
test_that("fsusieWrapper high-purity branch builds sets and computes cs_corr", {
    ...
    fake_fit <- list(
        ...
        alpha = list(
            matrix(
                rep(c(0.4, 0.4, 0.05, 0.05, 0.1), each = 2),
                nrow = 2,
                byrow = FALSE
            ),
            matrix(
                rep(c(0.05, 0.05, 0.45, 0.4, 0.05), each = 2),
                nrow = 2,
                byrow = FALSE
            )
        )
...
test_that("resolvePipColumn covers NULL/empty/method/pip/single/multi branches", {
    ...
    expect_equal(
        pecotmr:::resolvePipColumn(
            data.frame(pip_susie = 0.1, pip_x = 0.2),
            method = "susie"
        ),
        "pip_susie"
    )
    ...
    # plain "pip" present
    expect_equal(
        pecotmr:::resolvePipColumn(data.frame(pip = 0.1, pip_susie = 0.2)),
        "pip"
    )
    # single pip_ column, no "pip"
    expect_equal(
        pecotmr:::resolvePipColumn(data.frame(pip_susie = 0.1)),
        "pip_susie"
    )
    # multiple pip_ columns, no "pip", no method -> NULL (ambiguous)
    expect_null(pecotmr:::resolvePipColumn(data.frame(
        pip_a = 0.1,
        pip_b = 0.2
    )))
})
...
test_that("formatCsColumn formats integer and fractional coverage, errors on non-numeric", {
    ...
    expect_equal(
        pecotmr:::formatCsColumn(0.999, "susieInf"),
        "CS_99_9_susieInf"
    )
    expect_error(
        pecotmr:::formatCsColumn(NA, "susie"),
...
test_that(".translateLegacyCsColumnName converts cs_coverage_* and passes others through", {
    ...
    expect_equal(
        pecotmr:::.translateLegacyCsColumnName(c(
            "cs_coverage_0.95",
            "variant_id",
            "cs_coverage_0.7"
        )),
...
test_that(".setFinemappingFitClass assigns method classes and handles NULL/unknown", {
    ...
    expect_true(
        "susiF" %in%
            class(pecotmr:::.setFinemappingFitClass(list(a = 1), "fsusie"))
    )
    expect_true(
        "mvsusie" %in%
            class(pecotmr:::.setFinemappingFitClass(list(a = 1), "mvsusie"))
    )
    expect_true(
        "susieRss" %in%
            class(pecotmr:::.setFinemappingFitClass(
                list(a = 1),
                "singleEffect"
            ))
    )
...
test_that("prepareSusieFromInfArgs ash branch sets convergence, caps L_greedy, keeps preset refine", {
    fit <- list(V = rep(1, 5))
    a <- pecotmr:::prepareSusieFromInfArgs(
        list(L = 3, L_greedy = 10),
        fit,
        refineDefault = TRUE,
    ...
    b <- pecotmr:::prepareSusieFromInfArgs(
        list(refine = FALSE),
        fit,
        refineDefault = TRUE
    )
...
test_that(".asEffectMatrix handles NULL, list, matrix, and data.frame", {
    ...
    expect_equal(
        pecotmr:::.asEffectMatrix(list(c(1, 2), c(3, 4))),
...
test_that(".asLbfMatrix prefers lbf_variable, falls back to lBF, else NULL", {
    expect_equal(
        pecotmr:::.asLbfMatrix(list(lbf_variable = matrix(1, 2, 2))),
        matrix(1, 2, 2)
    )
    expect_equal(
        pecotmr:::.asLbfMatrix(list(lBF = matrix(2, 2, 2))),
        matrix(2, 2, 2)
    )
...
test_that(".parseGrange returns NA for NULL/empty/invalid and parses valid regions", {
    ...
    expect_equal(
        pecotmr:::.parseGrange("not_a_region"),
        c(start = NA_integer_, end = NA_integer_)
    )
    expect_equal(
        pecotmr:::.parseGrange("chr10:10823338-14348298"),
...
test_that("selectEffects returns integer(0) for empty alpha, V-filtered or all effects", {
    ...
    expect_equal(
        pecotmr:::selectEffects(list(
            alpha = matrix(0.2, 3, 4),
            V = c(1, 1e-20, 2)
        )),
        c(1L, 3L)
    )
...
test_that(".csPurityVec uses purity, falls back to cs_corr, else NA", {
    expect_equal(
        pecotmr:::.csPurityVec(list(
            sets = list(
                purity = data.frame(min.abs.corr = c(0.8, 0.9)),
                cs = list(1, 2)
            )
        )),
        c(0.8, 0.9)
...
test_that("extractVariantNames reads pip names, then alpha colnames, then a fallback", {
    expect_equal(
        pecotmr:::extractVariantNames(list(
            pip = setNames(c(0.1, 0.2), c("chr1:100:A:G", "chr1:200:C:T"))
        )),
...
test_that("trimFinemappingFit (susie) trims to selected effects and keeps scalar slots", {
    ...
    csTables <- list(
        list(
            sets = list(
                cs = list(L1 = c(1, 2)),
                purity = data.frame(min.abs.corr = 0.8)
            ),
            cs_corr = list(matrix(c(1, 0.8, 0.8, 1), 2)),
            pip = fit$pip
        ),
...
test_that("postprocessFinemappingFit.susiF post-processes an fsusie fit (empty-CS path)", {
    fit <- pecotmr:::.setFinemappingFitClass(
        list(
            pip = setNames(c(0.5, 0.3), c("chr1:100:A:G", "chr1:200:C:T")),
            alpha = matrix(0.5, 1, 2),
            mu = matrix(0.1, 1, 2),
            mu2 = matrix(0.02, 1, 2),
            cs = list()
        ),
...
test_that("postprocessFinemappingFits errors on empty or unnamed fit lists", {
    expect_error(
        postprocessFinemappingFits(list(), dataX = matrix(0, 2, 2)),
        "At least one fine-mapping fit"
    )
    expect_error(
        postprocessFinemappingFits(list(NULL), dataX = matrix(0, 2, 2)),
        "At least one fine-mapping fit"
    )
    expect_error(
...
test_that("formatFinemappingOutput errors when primaryMethod is absent", {
    post <- list(
        finemappingResults = list(susie = list(method = "susie")),
        top_loci = pecotmr:::.emptyTopLoci()
    )
    expect_error(
        formatFinemappingOutput(post, "nonexistent"),
        "primaryMethod was not found"
    )
})
...
test_that(".fmFitSusieRss forwards rssControl to susie_rss as control", {
    ...
    skip_if_not(
        "susie_rss_control" %in% getNamespaceExports("susieR"),
...
test_that("mvsusieWeights real fit returns p x K weights or errors on unstable small data", {
    ...
    res <- tryCatch(
        suppressMessages(mvsusieWeights(X = m$X, Y = m$Y, L = 5, LGreedy = 2)),
        error = function(e) e
    )
...
test_that("fsusieWeights matches fsusieR's own out_prep reconstruction (post_processing='none')", {
    ...
    for (l in seq_along(fit$fitted_wc)) {
        ...
        contrib <- colSums(
            (al * (1 / csdX) * as.matrix(fit$fitted_wc[[l]])) %*% S
        )
        maxErr <- max(
            maxErr,
            max(abs(contrib - as.numeric(fit$fitted_func[[l]])))
        )
    }
