...
test_that("getLbf returns the wide variant x effect lbf matrix", {
  ...
  lbf  <- matrix(c(3.0, 2.5, 0.1,
                   0.1, 0.1, 2.0), nrow = 2, byrow = TRUE)   # L=2 x p=3
  fit  <- list(alpha = matrix(0.2, 2, 3), mu = matrix(0.1, 2, 3),
               mu2 = matrix(1, 2, 3), pip = c(0.5, 0.4, 0.3), lbf_variable = lbf)
  class(fit) <- "susie"
  e <- FineMappingEntry(variantIds = vids, susieFit = fit,
                        topLoci = data.frame(variant_id = vids, pip = fit$pip,
...
test_that("getLbf aggregates over a collection, carrying identity + NA-filling ragged effects", {
  ...
  mkEntry <- function(L) {
    ...
    fit <- structure(list(lbf_variable = lbf, pip = rep(0.3, length(vids))),
                     class = "susie")
    FineMappingEntry(variantIds = vids, susieFit = fit,
                     topLoci = data.frame(variant_id = vids, pip = rep(0.3, length(vids)),
                                          stringsAsFactors = FALSE))
  }
  ...
  res <- QtlFineMappingResult(
    study = c("s1", "s1"), context = c("brain", "blood"),
    trait = c("g", "g"), method = c("susie", "susie"),
    entry = list(mkEntry(2L), mkEntry(3L)))
  w <- getLbf(res)
  expect_true(all(c("study", "context", "trait", "method", "variant_id") %in% names(w)))
