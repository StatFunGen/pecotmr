...
.make_two_study_gwasSumStats <- function(n = 3, withMaf = FALSE) {
    mkGr <- function() {
        gr <- GenomicRanges::GRanges(
            "chr1",
            IRanges::IRanges(
                start = seq(100, by = 100, length.out = n),
                width = 1
            )
        )
...
test_that("writeSumstatsVcf(GwasSumStats): emits the AF genotype field when MAF present", {
    ...
    ss <- GwasSumStats(
        ...
        entry = list(local({
            gr <- GenomicRanges::GRanges(
                "chr1",
                IRanges::IRanges(start = c(100, 200, 300, 400), width = 1)
            )
            S4Vectors::mcols(gr) <- S4Vectors::DataFrame(
                SNP = paste0("rs", 1:4),
                A1 = rep("A", 4),
...
test_that("writeSumstatsVcf(FineMappingResult): emits LBF / LFSR / PUR / fullFit FORMAT fields", {
    ...
    tl <- data.frame(
        ...
        lfsr = c(0.01, 0.2, 0.4),
        cs_95 = paste0("susie_", c(1L, 0L, 2L)),
        cs_95_purity = c(0.95, 0, 0.88),
        within_cs_pip = c(0.6, 0, 0.5),
        cs_logbf_susie_1 = c(2.2, NA, NA),
