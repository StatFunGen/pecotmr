...
.je_synthCell <- function(groups) {
    new(
        ...
        pattern = "context",
        dataForm = "individual",
        enumerate = function(data, scope, args) groups,
        minGroup = 2L
    )
}
...
test_that(".runJointCell: cross-context FM uses the per-fold mr.mash CV prior", {
    ...
    U <- list(K = diag(2))
    fp <- list(
        dataDrivenPriorMatrices = list(U = U, w = c(K = 1)),
        w0 = c(K_grid1 = 1),
        V = diag(2)
    )
    sp <- data.frame(
...
.je_mockTwasCv <- function(
    ...
    list(
        ...
        performance = list(
            mrmash_performance = matrix(
                0,
                ncol(Y),
                6,
                dimnames = list(colnames(Y), metric6)
            )
        ),
        foldFits = list(
            fold_1 = list(mrmash_weights = .je_fakeMrmashFit()),
...
.je_mkSsGroup <- function(tid, p = 3L, k = 2L) {
    ...
    new(
        ...
        conditions = data.frame(
            study = "S",
            context = paste0("c", seq_len(k)),
            trait = tid,
            stringsAsFactors = FALSE
        ),
        Z = Z,
        # The group carries the LD REFERENCE now; tests that fit mock
        # .ldFromSketch to supply the matrix (see .je_mockSsLd).
        ldSketch = NULL,
        N = c(100, 120)
    )
}

# Identity LD over whatever variants are asked for -- the shape .je_ssGroup
# used to hard-code into its R slot.
.je_mockSsLd <- function(sketch, vids, ...) {
    diag(length(vids)) |>
        `dimnames<-`(list(vids, vids))
}
...
test_that(".runJointCell: cross-context FM sumstats (mvsusie_rss) -> per-context", {
    # The group carries the LD reference; supply the matrix it derives.
    local_mocked_bindings(
        .ldFromSketch = .je_mockSsLd,
        .package = "pecotmr"
    )
    set.seed(6)
...
test_that(".lookupJointCell: present cells resolve, absent cells error", {
    for (df in c("individual", "sumstats")) {
        expect_s4_class(
            pecotmr:::.lookupJointCell("context", df),
            "JointDispatchCell"
        )
        expect_s4_class(
            pecotmr:::.lookupJointCell("trait", df),
            "JointDispatchCell"
        )
    }
    expect_s4_class(
        pecotmr:::.lookupJointCell("study", "sumstats"),
        "JointDispatchCell"
    )
    expect_error(
        pecotmr:::.lookupJointCell("study", "individual"),
...
test_that("fitJointGroup(Individual, Fm): fsusie without pos errors; unknown token errors", {
    ...
    expect_error(
        pecotmr:::fitJointGroup(grp, pipe, "bogus", list()),
        "unsupported token"
    )
})
...
.je_ensGroup <- function(n = 40L, nCond = 2L) {
    ...
    new(
        ...
        conditions = data.frame(
            study = "S",
            context = paste0("c", seq_len(nCond)),
            trait = "g",
            stringsAsFactors = FALSE
        ),
        X = matrix(
    ...
    expect_warning(
        pecotmr:::fitJointGroup(
            ...
            list(
                methodList = list(
                    mrmash_weights = list(
                        dataDrivenPriorMatrices = list(U = diag(2))
                    )
                )
            )
        ),
...
.je_mkSe <- function(traits = c("G1", "G2"), n = 6L) {
    rng <- GenomicRanges::GRanges(
        "chr1",
        IRanges::IRanges(
            start = seq(100L, by = 100L, length.out = length(traits)),
            width = 1L
        )
    )
    names(rng) <- traits
...
test_that(".enumCrossContextIndividual: one group per trait in >= 2 contexts", {
    ...
    local_mocked_bindings(
        ...
        .buildIndividualCrossContextXy = function(
            ...
            Y <- matrix(
                ...
                4,
                2,
                dimnames = list(paste0("s", 1:4), c("c1", "c2"))
            )
    ...
    expect_equal(as.character(.jgConditions(g[[1L]])$context), c("c1", "c2"))
    expect_equal(as.character(.jgConditions(g[[1L]])$trait), c("G1", "G1"))
})
...
test_that(".enumCrossContextIndividual: study not in scope / < 2 contexts -> empty", {
    ...
    expect_length(
        pecotmr:::.enumCrossContextIndividual(
            NULL,
            list(studies = "OTHER", contexts = list(), traits = list())
        ),
        0L
    )
    expect_length(
...
test_that(".enumCrossTraitIndividual: one group per context with >= 2 traits + pos", {
    ...
    local_mocked_bindings(
        ...
        .buildIndividualCrossTraitXy = function(
            ...
            Y <- matrix(
                ...
                4,
                2,
                dimnames = list(paste0("s", 1:4), c("G1", "G2"))
            )
    ...
    expect_length(g, 1L)
    expect_equal(as.character(.jgConditions(g[[1L]])$context), c("c1", "c1"))
    expect_equal(as.character(.jgConditions(g[[1L]])$trait), c("G1", "G2"))
    # one per TRAIT (rowRanges midpoints), not per variant
    expect_equal(.jgTraitPos(g[[1L]]), c(100, 200))
})
...
test_that(".enumCrossTraitIndividual: study not in scope -> empty", {
    ...
    expect_length(
        pecotmr:::.enumCrossTraitIndividual(
            NULL,
            list(studies = "X", contexts = list(), traits = list())
        ),
        0L
    )
})
...
test_that(".enumCrossTraitSumstats: groups per (study, context) with >= 2 traits", {
    ...
    expect_length(g, 1L)
    expect_equal(as.character(.jgConditions(g[[1L]])$trait), c("t1", "t2"))
...
test_that(".enumCrossStudySumstats: group per (context, trait) in >= 2 studies", {
    ...
    expect_length(g, 1L)
    expect_equal(as.character(.jgConditions(g[[1L]])$study), c("S1", "S2"))
...
test_that(".enumComposedIndividual: one group joining every (context, trait) tuple", {
    ...
    local_mocked_bindings(
        ...
        .buildComposedIndividualXy = function(
            ...
            Y <- matrix(
                ...
                4,
                3,
                dimnames = list(paste0("s", 1:4), c("c1:gA", "c1:gB", "c2:gA"))
            )
            X <- matrix(
                0,
                4,
    ...
    expect_length(g, 1L)
    expect_equal(
        as.character(.jgConditions(g[[1L]])$context),
        c("c1", "c1", "c2")
    )
    expect_equal(
        as.character(.jgConditions(g[[1L]])$trait),
        c("gA", "gB", "gA")
    )
})
...
test_that(".enumComposedIndividual: study not in scope / NULL xy -> empty", {
    ...
    expect_length(
        pecotmr:::.enumComposedIndividual(
            NULL,
            list(studies = "X")
        ),
        0L
...
test_that(".enumComposedSumstats: one group per fixed-axis row block", {
    ...
    g <- pecotmr:::.enumComposedSumstats(
        df,
        scope,
        args = list(axes = c("context", "trait"))
    )
    expect_length(g, 1L)
    expect_equal(as.character(.jgConditions(g[[1L]])$context), c("c1", "c2"))
})
...
test_that("fitJointGroup(SumStats, Fm): fsusie and unknown tokens error", {
    ...
    pipe <- new("FmJointPipeline", config = list(coverage = 0.95))
    expect_error(
        pecotmr:::fitJointGroup(grp, pipe, "fsusie", list()),
        "no RSS variant"
    )
    expect_error(
        pecotmr:::fitJointGroup(grp, pipe, "bogus", list()),
...
test_that("fitJointGroup(twas): spike-and-slab pi feeds bayes_b probIn", {
    ...
    local_mocked_bindings(
        mrashWeights = function(X, y, ...) {
            out <- matrix(0.05, ncol(X), 1L, dimnames = list(colnames(X), NULL))
            attr(out, "fit") <- list(pi = c(0.7, 0.2, 0.1))
            out
        },
...
test_that(".runJointCell: empty enumeration -> NULL", {
    emptyCell <- new(
        ...
        pattern = "context",
        dataForm = "individual",
        enumerate = function(data, scope, args) list(),
        minGroup = 2L
    )
    pipe <- new("FmJointPipeline", config = list(coverage = 0.95))
...
test_that(".runJointCell: a fitter returning all-NULL entries yields no rows -> NULL", {
    ...
    local_mocked_bindings(
        fitJointGroup = function(group, pipeline, token, args) {
            vector("list", nrow(.jgConditions(group)))
        },
        .package = "pecotmr"
    )
...
test_that(".twasEnsembleLayer: entries lacking CV predictions are skipped", {
    ...
    noCv <- list(twasWeightsRow(
        variantIds = colnames(.jgX(g)),
        weights = rnorm(ncol(.jgX(g))),
        cvResult = NULL
    ))
    ens <- pecotmr:::.twasEnsembleLayer(
        g,
...
test_that(".twasEnsembleLayer: unnamed ensemble weights fall back to a method's variant ids", {
    ...
    local_mocked_bindings(
        ensembleWeights = function(
            ...
            list(
                ensembleTwasWeights = as.numeric(rep(0.1, ncol(.jgX(g)))), # no names
                methodCoef = c(0.5, 0.5),
                methodPerformance = c(0.8, 0.7)
            )
        },
        .package = "pecotmr"
    )
...
test_that("construct: empty records -> NULL for both pipelines", {
    empty <- list()
    expect_null(pecotmr:::construct(
        new("FmJointPipeline", config = list()),
        empty
    ))
    expect_null(pecotmr:::construct(
        new("TwasJointPipeline", config = list()),
        empty
    ))
})
...
test_that(".runJointSpecs: no methods or no specs -> NULL", {
    ...
    expect_null(pecotmr:::.runJointSpecs(
        list(),
        NULL,
        "individual",
        pipe,
