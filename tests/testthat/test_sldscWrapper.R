...
test_that("readSldscFrq stacks per-chromosome .frq into one table", {
    dir <- withr::local_tempdir()
    for (chr in 1:2) {
        .make_frq(dir, chr, plinkName = "ref_chr")
    }
...
test_that("readSldscFrq falls back to a generic .frq glob when the prefix misses", {
    dir <- withr::local_tempdir()
    for (chr in 1:2) {
        .make_frq(dir, chr, plinkName = "other_chr")
    }
...
test_that("computeSldscAnnotSd errors with zero degrees of freedom", {
    ...
    expect_error(
        computeSldscAnnotSd(sd, mafCutoff = 0.05),
...
test_that("computeSldscMRef falls back to annot row count when frq absent and mafCutoff = 0", {
    expect_equal(
        computeSldscMRef(.sldscMkData(withFrq = FALSE), mafCutoff = 0),
        6L
    )
})
...
test_that("computeSldscMRef errors when mafCutoff > 0 but no frq data", {
    expect_error(
        computeSldscMRef(.sldscMkData(withFrq = FALSE), mafCutoff = 0.05),
        "requires frq data"
    )
})
...
.make_trait_data <- function(cats = c("A_0", "B_0"), nBlocks = 10, h2g = 0.3) {
    ...
    taus <- rep(1e-7, n)
    blocks <- matrix(
        rnorm(nBlocks * n, mean = rep(taus, each = nBlocks), sd = 1e-8),
        nrow = nBlocks,
        ncol = n
    )
...
.make_per_trait_meta <- function(
    ...
    if (is.null(means)) {
        means <- rnorm(nTraits, 1e-5, 1e-6)
    }
    if (is.null(ses)) {
        ses <- rep(1e-6, nTraits)
    }
    per_trait <- list()
...
.sms_makeWideTrait <- function() {
    list(
        summary = data.frame(
            target = c("A_0", "B_0"),
            tauStarSingle = c(2, 3),
            tauStarSeSingle = c(.5, .5),
            enrichmentSingle = c(1.5, 2),
            enrichmentSeSingle = c(.3, .3),
            enrichmentPSingle = c(.01, .02),
            enrichstatSingle = c(1, 1.2),
            enrichstatSeSingle = c(.2, .2),
            tauStarJoint = c(1.8, 2.5),
            tauStarSeJoint = c(.4, .4),
            enrichmentJoint = c(1.4, 1.9),
            enrichmentSeJoint = c(.3, .3),
            enrichmentPJoint = c(.02, .03),
            enrichstatJoint = c(.9, 1.1),
            enrichstatSeJoint = c(.2, .2),
            stringsAsFactors = FALSE
        )
    )
}
...
test_that("sldscSubsetMeta metas a chosen trait/category subset with the four blocks", {
    res <- list(
        per_trait = list(
            t1 = .sms_makeWideTrait(),
            t2 = .sms_makeWideTrait(),
...
test_that("sldscSubsetMeta defaults categories from params and validates traits", {
    ...
    expect_error(
        sldscSubsetMeta(list(per_trait = res$per_trait), c("t1")),
        "no `targetCategories`"
    )
})
...
test_that(".sldscAssembleTraitSummary combines single and joint", {
    ...
    single_df <- data.frame(
        ...
        tauSe = c(3e-8, 4e-8),
        tauStar = c(0.01, 0.02),
        tauStarSe = c(0.003, 0.004),
        enrichment = c(2.0, 3.0),
        enrichmentSe = c(0.4, 0.6),
        enrichmentP = c(0.01, 0.05),
        enrichstat = c(0.001, 0.002),
        enrichstatSe = c(0.0003, 0.0004),
        stringsAsFactors = FALSE
    )
    joint_df <- data.frame(
        target = targets,
        tau = c(1.1e-7, 2.1e-7),
        tauSe = c(3.1e-8, 4.1e-8),
        tauStar = c(0.011, 0.021),
        tauStarSe = c(0.0031, 0.0041),
        stringsAsFactors = FALSE
    )
    ...
    per_trait <- list(
        traitX = list(
            summary = data.frame(
                target = "A_0",
                tauStarSingle = 0.01,
                tauStarSeSingle = 0.003,
                enrichmentSingle = 2.0,
                enrichmentSeSingle = 0.4,
                stringsAsFactors = FALSE
            )
        )
    )
