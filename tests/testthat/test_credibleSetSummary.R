...
test_that("getCredibleSetSummary returns one row per CS with size/purity/V/logBF/lead", {
  ...
  lbf <- matrix(c(3.0, 2.5, 0.1, 0.1, 0.0,
                  0.1, 0.1, 2.0, 1.8, 0.0), L, P, byrow = TRUE)
  fit <- list(alpha = alpha, mu = matrix(0.3, L, P), mu2 = matrix(1.2, L, P),
              pip = c(0.6, 0.5, 0.4, 0.4, 0.02), V = c(0.8, 0.5),
              lbf_variable = lbf,
              sets = list(purity = data.frame(
                min.abs.corr = c(0.9, 0.3), mean.abs.corr = c(0.95, 0.4),
                row.names = c("L1", "L2"))))
  class(fit) <- "susie"
...
test_that("getCredibleSetSummary is empty when there are no credible sets", {
  ...
  fit <- list(alpha = matrix(c(0.2, 0.1), 1, 2), mu = matrix(0.1, 1, 2),
              mu2 = matrix(1, 1, 2), pip = c(0.05, 0.04))
  class(fit) <- "susie"
...
test_that("getCredibleSetSummary is empty when the requested coverage column is absent", {
  ...
  vn <- c("chr1:100:A:G", "chr1:200:C:T")
  tl <- data.frame(variant_id = vn, pip = c(0.7, 0.2), logBF = c(2.5, 0.3),
                   cs_95 = c("susie_1", "susie_1"), cs_95_purity = c(0.9, 0.9),
...
test_that("getCredibleSetSummary reports NA cs_log_bf when the fit carries no lbf matrix", {
  ...
  vn <- c("chr1:100:A:G", "chr1:200:C:T")
  tl <- data.frame(variant_id = vn, pip = c(0.7, 0.2), logBF = c(2.5, 0.3),
                   cs_95 = c("susie_1", "susie_1"), cs_95_purity = c(0.88, 0.88),
                   stringsAsFactors = FALSE)
  e  <- FineMappingEntry(variantIds = vn, susieFit = list(trimmed = TRUE), topLoci = tl)
  s  <- getCredibleSetSummary(e)
...
test_that("getCredibleSetSummary reports NA cs_log_bf when the effect's lbf row is all non-finite", {
  ...
  tl  <- data.frame(variant_id = vn, pip = c(0.7, 0.2), logBF = c(1.0, 0.3),
                    cs_95 = c("susie_1", "susie_1"), cs_95_purity = c(0.9, 0.9),
                    stringsAsFactors = FALSE)
...
test_that("getCredibleSetSummary aggregates across a collection with entry identity", {
  ...
  res <- QtlFineMappingResult(study = c("s", "s"), context = c("brain", "blood"),
                              trait = c("g", "g"), method = c("susie", "susie"),
                              entry = list(mkEntry(), mkEntry()))
  s <- getCredibleSetSummary(res)
  expect_true(all(c("study", "context", "trait", "method", "cs") %in% names(s)))
