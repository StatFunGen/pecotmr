...
# ===========================================================================
# Strategy: mock computeQtlEnrichment so the pipeline runs end-to-end on a
...
# ===========================================================================
# Per-study / per-context iteration via mocked computeQtlEnrichment
# ===========================================================================

test_that("qtlEnrichmentPipeline: returns one row per (gwasStudy, qtlContext) pair", {
  gfmr <- .qep_makeGwasFmr()
  qfmr <- .qep_makeQtlFmr(contexts = c("c1", "c2"))
  local_mocked_bindings(computeQtlEnrichment = .qep_mockEnrichment(2.0),
                        .package = "pecotmr")
...
test_that("qtlEnrichmentPipeline: computeQtlEnrichment failure produces a warning + skip", {
  gfmr <- .qep_makeGwasFmr()
  qfmr <- .qep_makeQtlFmr()
  local_mocked_bindings(
    computeQtlEnrichment = function(...) stop("synthetic failure"),
    .package = "pecotmr")
  expect_warning(
    out <- qtlEnrichmentPipeline(gwasFineMappingResult = gfmr,
                                  qtlFineMappingResult  = qfmr),
    "computeQtlEnrichment failed"
  )
