...
test_that("twasWeightsPipeline(QtlSumStats): runs end-to-end with mocked solvers", {
    ...
    res <- suppressMessages(suppressWarnings(
        twasWeightsPipeline(ss, methods = c("mrash", "lasso"), verbose = 0)
    ))
    expect_s4_class(res, "TwasWeights")
...
test_that("twasWeightsPipeline(QtlSumStats): empty contexts/trait filter errors", {
    ss <- .tp_makeQtlSumStats()
    expect_error(
        twasWeightsPipeline(ss, methods = "lasso", contexts = "ghost"),
        "no entries matched"
    )
})
...
test_that("gate: QtlSumStats + susie + fineMappingResult threads the susieRssFit", {
    ...
    mocks <- list(
        ...
        susieRssWeights = function(stat, LD, susieRssFit = NULL, ...) {
            if (!is.null(susieRssFit)) {
                fitsSeen <<- fitsSeen + 1L
            }
            rep(0, nrow(LD))
        }
    )
    ...
    mocks <- c(
        ...
        list(
            lassoWeights = function(X, y, ...) {
                lassoCalls <<- lassoCalls + 1L
                rep(0, ncol(X))
            },
            enetWeights = function(X, y, ...) {
                enetCalls <<- enetCalls + 1L
                rep(0, ncol(X))
            }
        )
    )
...
test_that(".twasCacheLookup: non-TwasWeights input returns NULL", {
    expect_null(pecotmr:::.twasCacheLookup(
        "not_a_tw",
        "s1",
        "c1",
        "t1",
...
test_that("ensembleWeights: invalid context_index errors", {
    ...
    expect_error(
        ensembleWeights(cv, Y = cv$.y, contextIndex = "a"),
        "contextIndex"
    )
})
...
test_that("ensembleWeights: aligns Y and predictions by sample name", {
    ...
    expect_equal(
        res_aligned$methodCoef,
        res_original$methodCoef,
        tolerance = 1e-10
    )
})
...
test_that("ensembleWeights: ensembleTwasWeights is sum of zeta_k * w_k", {
    ...
    expect_equal(
        as.numeric(res$ensembleTwasWeights),
        as.numeric(expected),
        tolerance = 1e-10
    )
})
...
for (slv in c("quadprog", "nnls", "lbfgsb", "glmnet")) {
    test_that(
        paste0(
            "ensembleWeights: solver='",
            slv,
            "' produces valid coefficients"
        ),
        {
            if (slv == "quadprog") {
    ...
    test_that(
        paste0(
            "ensembleWeights: solver='",
            slv,
            "' assigns best method largest coef"
        ),
        {
            if (slv == "quadprog") {
                skip_if_not_installed("quadprog")
    ...
    test_that(
        ...
        ),
        {
            ...
            for (k in seq_along(cv$.method_names)) {
                m <- cv$.method_names[k]
                expected <- expected +
                    res$methodCoef[m] * wt[[paste0(m, "_weights")]]
            }
            expect_equal(
                as.numeric(res$ensembleTwasWeights),
                as.numeric(expected),
...
test_that(".twasCheckMethodCapabilities: empty token list is a no-op", {
    expect_silent(pecotmr:::.twasCheckMethodCapabilities(
        character(0),
        "QtlDataset"
    ))
})
...
test_that(".twasCheckMultivariateY: no multivariate tokens is a no-op", {
    expect_silent(pecotmr:::.twasCheckMultivariateY(
        c("lasso", "enet"),
        nTraits = 1L,
        nContexts = 1L
    ))
...
test_that(".twasCheckMultivariateY: mvsusie/mrmash error with single trait, single context", {
    expect_error(
        pecotmr:::.twasCheckMultivariateY(
            c("mvsusie", "mrmash"),
            nTraits = 1L,
            nContexts = 1L
        ),
...
.gsd_makeGwasSumStats <- function(mcolsDf, snp_n = nrow(mcolsDf)) {
    gr <- GenomicRanges::GRanges(
        ...
        ranges = IRanges::IRanges(
            start = seq(100L, by = 100L, length.out = snp_n),
            width = 1L
        )
    )
...
test_that("estimateSparsity: legacy list input reads attr(.,'fit')$pi", {
    ...
    expect_equal(estimateSparsity(weightResults), 1 - 0.6, tolerance = 1e-12)
})
...
test_that(".twasRegionLabel / .twasFitsForRegion select per-region fits", {
    ...
    expect_length(
        pecotmr:::.twasFitsForRegion(
            list(susie = list(alpha = 1, pip = 2)),
            1L,
            2L
        ),
        0L
...
test_that(".twasCvResultFor merges per-method cvResult across a tuple's entries", {
    ...
    mkCv <- function(key) {
        list(
            ...
            prediction = setNames(
                list(matrix(0, 2, 1)),
                paste0(key, "_predicted")
            ),
            performance = setNames(
                list(matrix(0, 1, 6)),
                paste0(key, "_performance")
            )
        )
    }
...
test_that(".twasFineMappingFitFor: NULL fineMappingResult -> NULL", {
    expect_null(pecotmr:::.twasFineMappingFitFor(
        NULL,
        "S",
        "c1",
        "t1",
...
test_that(".solveEnsembleQuadprog: solver failure and all-zero solution fall back to equal weights", {
    ...
    y <- rnorm(10)
    local_mocked_bindings(
        solve.QP = function(...) stop("boom"),
        .package = "pecotmr"
    )
    expect_warning(
        z <- pecotmr:::.solveEnsembleQuadprog(P, y, 2L),
        "QP solver failed"
    )
    expect_equal(z, c(0.5, 0.5))
    local_mocked_bindings(
        solve.QP = function(...) list(solution = c(0, 0)),
        .package = "pecotmr"
    )
...
test_that(".solveEnsembleLbfgsb: solver failure and all-zero solution fall back to equal weights", {
    ...
    expect_warning(
        z <- pecotmr:::.solveEnsembleLbfgsb(P, y, 2L),
        "L-BFGS-B solver failed"
    )
    expect_equal(z, c(0.5, 0.5))
    local_mocked_bindings(
        optim = function(...) list(par = c(0, 0)),
        .package = "pecotmr"
    )
...
test_that("ensembleWeights: multi-dataset input validation errors", {
    ...
    y <- rnorm(30)
    expect_error(
        ensembleWeights(cvResults = list(), Y = list(y)),
        "non-empty list"
    )
    expect_error(
        ensembleWeights(
            cvResults = list(list(prediction = pb), list(prediction = pb)),
            Y = list(y, y),
            twasWeightList = list(list())
        ), # wrong length
        "must be a list of the same length"
    )
    expect_error(
        ensembleWeights(
            cvResults = list(list(prediction = pb), list(foo = 1)),
            Y = list(y, y)
        ),
        "does not contain"
    )
...
test_that("ensembleWeights: unnamed prediction list errors", {
    ...
    names(pb) <- NULL
    expect_error(
        ensembleWeights(cvResults = list(prediction = pb), Y = rnorm(30)),
...
test_that("ensembleWeights: too few complete observations errors", {
    ...
    pb <- list(
        a_predicted = matrix(rnorm(2), 2, 1, dimnames = list(samp, NULL)),
...
test_that("ensembleWeights: only one method with signal -> it gets full weight", {
    ...
    pb <- list(
        ...
        b_predicted = matrix(0, 30, 1, dimnames = list(samp, NULL))
    ) # flat
    res <- suppressMessages(ensembleWeights(
        cvResults = list(prediction = pb),
        Y = y
    ))
...
test_that(".twasCheckFineMappingMethods: empty token list is a no-op", {
    expect_null(pecotmr:::.twasCheckFineMappingMethods(
        character(0),
        NULL,
        "QtlDataset"
    ))
...
test_that("twasWeightsPipeline(QtlSumStats): jointSpec mr.mash-only with no fits errors", {
    ss <- .tp_makeQtlSumStats()
    local_mocked_bindings(
        .twasDispatchJointSpecsQtlSumStats = function(...) NULL,
        .package = "pecotmr"
    )
    expect_error(
        suppressMessages(twasWeightsPipeline(
...
test_that("ensembleWeights: partial sample overlap emits a message", {
    samp <- paste0("s", 1:30)
    pb <- list(
        a_predicted = matrix(rnorm(30), 30, 1, dimnames = list(samp, NULL)),
...
test_that(".solveEnsembleNnls: solver failure and all-zero fall back to equal weights", {
    ...
    expect_warning(
        z <- pecotmr:::.solveEnsembleNnls(P, y, 2L),
        "NNLS solver failed"
    )
    expect_equal(z, c(0.5, 0.5))
    local_mocked_bindings(
        nnls = function(...) list(x = c(0, 0)),
        .package = "nnls"
    )
...
test_that(".solveEnsembleGlmnet: solver failure falls back to equal weights", {
    ...
    y <- rnorm(10)
    local_mocked_bindings(
        cv.glmnet = function(...) stop("boom"),
        .package = "glmnet"
    )
    expect_warning(
        z <- pecotmr:::.solveEnsembleGlmnet(P, y, 2L),
