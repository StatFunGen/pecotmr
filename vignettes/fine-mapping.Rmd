...
`pecotmr` exposes four SuSiE-family fine-mapping variants. Pick which to
fit via the `methods` parameter on `univariate_analysis_pipeline()`
...
All four populate the same 22-column `top_loci` table. When you fit
...
When `methods` is set explicitly, `add_susie_inf` only controls whether
the chained-init shortcut is applied to a `c("susie_inf", "susie")`
pair. Set `add_susie_inf = FALSE` with both methods present to fit
SuSiE-inf and SuSiE independently rather than chained.
