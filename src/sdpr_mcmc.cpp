...
void MCMC_state::update_p() {
    ...
    for (size_t i = 0; i < M; i++) {
        log_p[i] = std::log(static_cast<float>(p[i]) + 1e-40f);
    }
}
...
std::unordered_map<std::string, arma::vec> mcmc(
    ...
    MCMC_state state(data.beta_mrg.size(), M, a0k, b0k, sz, seed, legacy_random_init);
