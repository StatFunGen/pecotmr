...
#' alternating-least-squares solver by default; \code{lambda} defaults to
#' a sensible point on the lambda grid (\code{0.5 * lambda0(x)}) when not
#' supplied. Columns with all entries missing are removed before fitting
#' and a one-line message records the count.
#'
#' @param data Numeric matrix (samples x features) with missing values
#'   (NA).
#' @param rank.max Maximum rank of the SVD solution. Default
#'   \code{min(dim(data) - 1)}.
#' @param lambda Nuclear-norm penalty. NULL (default) chooses
#'   \code{0.5 * lambda0(data)} so the solution is not degenerate.
...
softimpute_imputation <- function(data, rank.max = NULL, lambda = NULL,
  ...
  if (is.null(lambda)) lambda <- 0.5 * softImpute::lambda0(X)
