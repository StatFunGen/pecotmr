...
#   Bonferroni -> stats::p.adjust(method="bonferroni", n = <per-gene test count>)
...
.qapSafeQvalue <- function(p) {
  if (!requireNamespace("qvalue", quietly = TRUE)) {
    ...
    stop("qtlAssociationPostprocess: the 'qvalue' package is required for ",
  ...
  tryCatch(
    ...
    error = function(e) {
      if (grepl("missing or infinite", conditionMessage(e))) {
        ...
        qvalue::qvalue(p, lambda = lambdaSeq, pi0.method = "bootstrap")$qvalues
      } else {
        stop("qtlAssociationPostprocess: qvalue::qvalue failed (", conditionMessage(e),
...
.qapGlobalAdjust <- function(eventP) {
  list(fdr = stats::p.adjust(eventP, method = "fdr"),
...
.qapPermutationNominalThreshold <- function(pBeta, qBeta, shape1, shape2,
                                            fdrThreshold) {
  ...
  if (length(lb) == 0L) return(out)                # no significant events
...
.qapFilterKeep <- function(mc, mafCutoff, cisWindow, afCol, nVar) {
  ...
  if (cisWindow > 0 && !is.null(mc$tss_distance) && !is.null(mc$tes_distance))
    keep <- keep & (mc$tss_distance >= -cisWindow & mc$tes_distance <= cisWindow)
  keep
}
...
.qapSignificanceMask <- function(x, method, threshold = NULL) {
  ...
  if (is.null(recipe))
    stop("This QtlSumStats was not produced by qtlAssociationPostprocess(); ",
         "no significance recipe to derive from.")
  if (is.null(threshold)) threshold <- recipe$fdrThreshold
  pcol <- recipe$pvalueCol; afcol <- recipe$afCol
  nrows <- nrow(x)
  masks <- lapply(seq_len(nrows), function(i)
    rep(FALSE, length(S4Vectors::mcols(x$entry[[i]])[[pcol]])))

  if (method == "permutation") {
    if (is.null(x$p_nominal_threshold))
      stop("getSignificantQtls: no p_nominal_threshold (run the permutation method).")
  ...
  } else if (method %in% c("bonferroni_original", "bonferroni_filtered")) {
    ...
    if (is.null(x[[fdrCol]]))
      stop("getSignificantQtls: '", method, "' columns absent; recompute with the ",
           "matching flavour.")
    sig <- as.numeric(x[[fdrCol]]) < threshold; sig[is.na(sig)] <- FALSE
    if (any(sig)) {
      varThr <- max(as.numeric(x[[pMinCol]])[sig], na.rm = TRUE)   # global scalar
      nVar   <- as.numeric(x[[nCol]])
      for (i in seq_len(nrows)) {
        mc <- S4Vectors::mcols(x$entry[[i]]); pv <- mc[[pcol]]
        keep <- pmin(1, pv * nVar[i]) <= varThr
        if (flav == "filtered")
          keep <- keep & .qapFilterKeep(mc, recipe$mafCutoff, recipe$cisWindow,
                                        afcol, length(pv))
        masks[[i]] <- keep & !is.na(keep)
      }
    }
  } else if (method == "qvalue") {
    qCol <- if (!is.null(x$q_beta)) "q_beta" else "q_bonferroni_min_original"
    if (is.null(x[[qCol]]))
      stop("getSignificantQtls: no event q-value column (q_beta / q_bonferroni_min_original).")
    sig <- as.numeric(x[[qCol]]) < threshold; sig[is.na(sig)] <- FALSE
    for (i in seq_len(nrows)) {
...
#' @export
setMethod("getSignificantQtls", "QtlSumStats",
  function(x, method = c("permutation", "bonferroni_original",
    ...
    if (length(pieces) == 0L) return(x$entry[[1L]][0L])
    do.call(c, pieces)
  }
)
...
# plain DFrame, add the columns, and re-wrap (the same rebuild idiom as subsetChr).
.qapRebuild <- function(x, newCols, qcInfo) {
  df <- methods::as(x, "DFrame")
  for (nm in names(newCols)) df[[nm]] <- newCols[[nm]]
  obj <- methods::new("QtlSumStats", df,
                      ldSketch = x@ldSketch, genome = x@genome, qcInfo = qcInfo)
...
#'   \code{*_filtered} columns are produced (requires the \code{n_variants_filtered}
#'   row column).
...
#'   no stored column -- it is a significance query (see \code{getSignificantQtls}).
#' @param pvalueCol,afCol Entry mcol names for the per-variant p-value / allele
#'   frequency (defaults \code{"P"} / \code{"af"}).
#' @export
setMethod("qtlAssociationPostprocess", "QtlSumStats",
  function(x, fdrThreshold = 0.05, mafCutoff = 0, cisWindow = 0,
           methods = c("permutation", "bonferroni"),
           pvalueCol = "P", afCol = "af") {
    methods   <- match.arg(methods, several.ok = TRUE)
    n         <- nrow(x)
    filtering <- (mafCutoff > 0 || cisWindow > 0)
    newCols   <- list()

    # ---- local Bonferroni: per-gene min adjusted p (original [+ filtered]) ----
    if ("bonferroni" %in% methods) {
      if (is.null(x$n_variants))
        stop("qtlAssociationPostprocess: `n_variants` row column is required ",
             "for the Bonferroni correction.")
      nVar     <- as.numeric(x$n_variants)
      nVarFilt <- if (!is.null(x$n_variants_filtered))
        as.numeric(x$n_variants_filtered) else NULL
      if (filtering && is.null(nVarFilt))
        stop("qtlAssociationPostprocess: `n_variants_filtered` row column is ",
      ...
      for (i in seq_len(n)) {
        mc <- S4Vectors::mcols(x$entry[[i]])
        pv <- mc[[pvalueCol]]
        if (is.null(pv) || length(pv) == 0L) next
        # The upstream p-value pre-filter always retains the global-min variant,
        # so min over the (possibly pre-filtered) entry is the exact per-gene min.
        pBonfOrig[i] <- min(stats::p.adjust(pv, method = "bonferroni", n = nVar[i]))
        if (filtering) {
          keep <- .qapFilterKeep(mc, mafCutoff, cisWindow, afCol, length(pv))
          if (any(keep))
            pBonfFilt[i] <- min(stats::p.adjust(pv[keep], method = "bonferroni",
                                                n = nVarFilt[i]))
        }
      }

      newCols$p_bonferroni_min_original <- pBonfOrig
      gaO <- .qapGlobalAdjust(pBonfOrig)
      newCols$fdr_bonferroni_min_original <- gaO$fdr
      newCols$q_bonferroni_min_original   <- gaO$q
      if (filtering) {
        newCols$p_bonferroni_min_filtered <- pBonfFilt
        gaF <- .qapGlobalAdjust(pBonfFilt)
        newCols$fdr_bonferroni_min_filtered <- gaF$fdr
        newCols$q_bonferroni_min_filtered   <- gaF$q
      }
    }

    # ---- permutation: BH-FDR of p_beta + per-gene nominal p threshold ----
    if ("permutation" %in% methods && !is.null(x$p_beta)) {
      pBeta <- as.numeric(x$p_beta)
      qBeta <- if (!is.null(x$q_beta)) as.numeric(x$q_beta) else .qapSafeQvalue(pBeta)
      if (is.null(x$q_beta)) newCols$q_beta <- qBeta
      newCols$fdr_beta <- stats::p.adjust(pBeta, method = "fdr")
      if (!is.null(x$beta_shape1) && !is.null(x$beta_shape2)) {
        newCols$p_nominal_threshold <- .qapPermutationNominalThreshold(
          pBeta, qBeta, as.numeric(x$beta_shape1),
          as.numeric(x$beta_shape2), fdrThreshold)
      }
    }

    # Stash the correction recipe so getSignificantQtls / annotateSignificance
    # can reproduce significance cheaply (thresholds, not flags).
    qc <- x@qcInfo
    qc$associationPostprocess <- list(
      fdrThreshold = fdrThreshold, mafCutoff = mafCutoff,
      cisWindow = cisWindow, methods = methods,
      pvalueCol = pvalueCol, afCol = afCol)
