...
.fmrRowsMatching <- function(x, study = NULL, context = NULL, trait = NULL,
                             method = NULL, region = NULL) {
  keys <- list(study = study, context = context, trait = trait,
               method = method, region_id = region)
...
.validateRegionColumn <- function(object) {
  if (!("region" %in% names(object))) return(character(0))
...
.naLikeColumn <- function(exemplar, n) {
  if (methods::is(exemplar, "GRanges"))
    return(GenomicRanges::GRanges(rep("chrUn", n),
                                  IRanges::IRanges(start = 1L, width = 0L)))
  if (is.character(exemplar)) return(rep(NA_character_, n))
  rep(NA, n)
}
...
# short-circuits to `onSingle(entry)` -- the bare per-entry view with no identity
# columns -- preserving the single-entry contract. A no-selector call always
...
.fmrAggregateView <- function(x, study = NULL, context = NULL, trait = NULL,
                              method = NULL, region = NULL, perEntry,
                              onSingle = perEntry) {
  anySelector <- !is.null(study) || !is.null(context) || !is.null(trait) ||
                 !is.null(method) || !is.null(region)
  sel <- NULL
  if (anySelector) {
    sel <- tryCatch(
      ...
      error = function(e) e)
    if (!inherits(sel, "error")) return(onSingle(sel))
  }
  idx <- .fmrRowsMatching(x, study = study, context = context, trait = trait,
                          method = method, region = region)
  ...
  if (length(idx) == 0L) {
    ...
    if (inherits(sel, "error")) stop(conditionMessage(sel))
  ...
  parts <- parts[!vapply(parts, is.null, logical(1L))]
  if (length(parts) == 0L) return(meta[integer(0), , drop = FALSE])
  .rbindAligned(parts)
}
