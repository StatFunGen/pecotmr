...
.isFsusieFit <- function(fit) {
  !is.null(fit) && is.list(fit) &&
    !is.null(fit$fitted_wc2) && !is.null(fit$fitted_func) &&
    !is.null(fit$outing_grid) && !is.null(fit$alpha)
}
...
.fsusiePopulateCredibleBand <- function(fit) {
  fn <- tryCatch(get("update_cal_credible_band.susiF", envir = asNamespace("fsusieR")),
...
.fsusieChrom <- function(fit) {
  vid <- names(fit$csd_X)
  if (is.null(vid) || length(vid) == 0L) return(NA_character_)
  tryCatch(parseVariantId(vid[1])$chrom, error = function(e) NA_character_)
}

.emptyCredibleBand <- function() {
  data.frame(cs = character(), chrom = character(), pos = numeric(),
             effect = numeric(), lower = numeric(), upper = numeric(),
             stringsAsFactors = FALSE)
}

# @noRd
.fsusieCredibleBandFit <- function(fit) {
  if (!.isFsusieFit(fit)) return(.emptyCredibleBand())
  ...
  parts <- lapply(seq_along(fit$cred_band), function(l) {
    ...
    if (is.null(band) || is.null(eff)) return(NULL)
    data.frame(cs = paste0("fsusie_", l), chrom = chrom, pos = grid,
               effect = as.numeric(eff),
               lower = as.numeric(band[2, ]), upper = as.numeric(band[1, ]),
               stringsAsFactors = FALSE)
  })
  parts <- parts[!vapply(parts, is.null, logical(1))]
  if (length(parts) == 0L) return(.emptyCredibleBand())
  do.call(rbind, parts)
}

# Map each fit-effect index (1..L, as affected_reg's `CS`) to pecotmr's canonical
# CS label + purity. The raw fSuSiE fit stores no purity (fsusieR computes it
# from the genotype matrix, which the fit does not retain), so the canonical
# value is `cs_95_purity` in the entry's topLoci. Matching is by the CS's variant
# MEMBERSHIP (not by index), which is robust to pecotmr's CS filtering/renumbering.
# @noRd
.fsusieCsMapFromTopLoci <- function(fit, topLoci) {
  ...
  if (is.null(topLoci) || is.null(vids) ||
  ...
  for (lab in retained) {
    ...
    for (l in seq_len(L)) {
      fv <- fit$cs[[l]]
      if (is.numeric(fv)) fv <- vids[as.integer(fv)]
      fv <- as.character(fv)
...
.fsusieAffectedRegionsFit <- function(fit, topLoci = NULL) {
  if (!.isFsusieFit(fit)) return(GenomicRanges::GRanges())
  fit <- .fsusiePopulateCredibleBand(fit)
  reg <- tryCatch(fsusieR::affected_reg(fit), error = function(e) NULL)
  if (is.null(reg) || nrow(reg) == 0L) return(GenomicRanges::GRanges())
  ...
  direction <- vapply(seq_len(nrow(reg)), function(i) {
    inRange <- which(grid >= reg$Start[i] & grid <= reg$End[i])
    eff <- fit$fitted_func[[reg$CS[i]]]
    if (length(inRange) == 0L || is.null(eff)) return(NA_character_)
    s <- sign(mean(as.numeric(eff)[inRange], na.rm = TRUE))
    if (is.na(s) || s == 0) NA_character_ else if (s > 0) "pos" else "neg"
  }, character(1))
  GenomicRanges::GRanges(
    seqnames = paste0("chr", sub("^chr", "", chrom)),
    ranges = IRanges::IRanges(start = as.integer(reg$Start), end = as.integer(reg$End)),
    cs = unname(csMap$label[csKey]), purity = unname(csMap$purity[csKey]),
    direction = direction)
}
...
#' @export
setGeneric("fsusieCredibleBand", function(x, ...) standardGeneric("fsusieCredibleBand"))

#' @rdname fsusieCredibleBand
#' @export
setMethod("fsusieCredibleBand", "FineMappingEntry",
  function(x, ...) .fsusieCredibleBandFit(getSusieFit(x)))

#' @rdname fsusieCredibleBand
#' @export
setMethod("fsusieCredibleBand", "FineMappingResultBase",
  function(x, ...) .fmrAggregateView(x, perEntry = function(e) fsusieCredibleBand(e)))
...
#' label, its purity, and the effect \code{direction} (\code{"pos"}/\code{"neg"};
...
#' @export
setGeneric("fsusieAffectedRegions", function(x, ...) standardGeneric("fsusieAffectedRegions"))

#' @rdname fsusieAffectedRegions
#' @export
setMethod("fsusieAffectedRegions", "FineMappingEntry",
  function(x, ...) .fsusieAffectedRegionsFit(getSusieFit(x), topLoci = x@topLoci))
...
setMethod("fsusieAffectedRegions", "FineMappingResultBase", function(x, ...) {
  grs <- lapply(seq_len(nrow(x)), function(i) {
    ...
    if (length(gr) > 0L) {
      ...
      if ("context" %in% names(x)) S4Vectors::mcols(gr)$context <- as.character(x$context[[i]])
      if ("trait"   %in% names(x)) S4Vectors::mcols(gr)$trait   <- as.character(x$trait[[i]])
    }
    gr
  })
  grs <- grs[vapply(grs, length, integer(1)) > 0L]
  if (length(grs) == 0L) return(GenomicRanges::GRanges())
  do.call(c, grs)
})
