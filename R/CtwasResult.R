  ...
  validity = function(object) {
    ...
    if (length(errors) == 0L) {
      if (length(object$entry) != nrow(object))
        errors <- c(errors, "length(entry) must equal nrow(.) for CtwasResult")
      entryTypes <- vapply(object$entry,
                           function(e) methods::is(e, "CtwasResultEntry"),
                           logical(1))
      if (!all(entryTypes))
        errors <- c(errors,
          "every element of the `entry` column must be a CtwasResultEntry")
      jointCols <- intersect(c("jointStudies", "jointContexts"), names(object))
      for (jc in jointCols) {
        if (!is.character(object[[jc]]))
          errors <- c(errors, sprintf("'%s' column must be character (got %s)",
                                      jc, class(object[[jc]])[[1L]]))
      }
      ...
      if (anyDuplicated(keyDf))
        errors <- c(errors,
          "(gwasStudy, study, context, method[, joint*]) tuple uniqueness violated")
    }
...
#' @param jointStudies Optional character vector (length \code{length(gwasStudy)})
#'   listing the joined studies for a multi-study run, \code{NA_character_}
#'   otherwise. \code{NULL} (default) omits the column.
...
#' @export
CtwasResult <- function(gwasStudy, study, context, method, entry,
                        jointStudies = NULL, jointContexts = NULL) {
  n <- length(gwasStudy)
  if (length(study) != n || length(context) != n || length(method) != n ||
      length(entry) != n) {
    stop("`gwasStudy`, `study`, `context`, `method`, and `entry` must all ",
...
#' @export
setMethod("getMethodNames", "CtwasResult",
          function(x) unique(as.character(x$method)))

#' @rdname getStudy
#' @export
setMethod("getStudy", "CtwasResult",
          function(x) unique(as.character(x$study)))

#' @rdname getContexts
#' @export
setMethod("getContexts", "CtwasResult",
...
.ctwasAggregateRows <- function(x, getter) {
  ...
  parts <- lapply(seq_len(n), function(i) {
    ...
    tb <- as.data.frame(tb)
    id <- data.frame(gwasStudy = as.character(x$gwasStudy)[[i]],
...
#' @rdname getFinemap
#' @export
setMethod("getFinemap", "CtwasResult",
          function(x, ...) .ctwasAggregateRows(x, getFinemap))

#' @rdname getSusieAlpha
#' @export
setMethod("getSusieAlpha", "CtwasResult",
          function(x, ...) .ctwasAggregateRows(x, getSusieAlpha))

#' @export
setMethod("show", "CtwasResult", function(object) {
  cat(sprintf("CtwasResult: %d run(s)\n", nrow(object)))
  if (nrow(object) > 0L)
    cat(sprintf("  %d GWAS stud(y/ies), %d method(s): %s\n",
